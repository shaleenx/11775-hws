...
def get_for_one(line, resnet_model, layer, scaler, normalize, to_tensor):
    ...
    try:
        cnn_feats = np.vstack([get_vector(img, resnet_model, scaler, normalize, to_tensor) for img in keyframe_iterator])
...
if __name__ == '__main__':
    ...
    #     # # get_cnn_features_from_video(downsampled_video_filename, cnn_feat_video_filename, keyframe_interval)
    #     # keyframe_iterator = get_keyframes(cnn, downsampled_video_filename, keyframe_interval)
    #     # try:
    #     #     cnn_feats = np.vstack(np.array(list(keyframe_iterator)))
    #     #     np.savetxt(cnn_feat_video_filename, cnn_feats, delimiter=",")
    #     #     print("Finished", video_name, "in", time.time() - start, "seconds")
    #     # except(ValueError):
